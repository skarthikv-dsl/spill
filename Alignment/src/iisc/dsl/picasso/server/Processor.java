	...
	public void run() {
		...
		try{
			...
			case MessageIds.RESUME_PROCESSING:
				...
				break;
			case MessageIds.RQEP_CREATE_TEMP_TABLES:
				/* Added by Nila - RQEP */
				ServerMessageUtil.sendStatusMessage(sock,reader,writer,0,"RQEP_CREATE_TEMP_TABLES - Start");
				// First Try Creating a Table.
				/*
				Statement stmt = database.createStatement();
				stmt.executeUpdate("create table RQEP_TEST(no numeric(1), name varchar(20))");
				*/
				
				String[][] replyRelationNames;  //[dimension][selectivity] - [0][5,95] increasing order
				
				System.out.println("Processing message");
				//ArrayList points1 = (ArrayList) clientPacket.get("PlanPoints");
				int i;
				Query queryT = Query.getQuery(qp, database);
				queryT.genConstants(qp.getResolution(), qp.getDistribution(), qp.getStartPoint(), qp.getEndPoint());
				int dim = qp.getDimension();
				int res[] = qp.getResolution();
				double [] sel1 = new double[dim];
				double [] sel2 = new double[dim];
				String select1 = new String("_MIN");
				String select2 = new String("_MAX");
				
				replyRelationNames = new String[dim][2];
				
				/*for(Iterator it = points1.iterator();it.hasNext();) {
					Integer ii = (Integer) it.next();
					int pt= ii.intValue();
					*/
					for(i = 0;i < dim;i ++) {
						//select[i] = qp.getStartPoint(i)/((qp.getEndPoint(i) - qp.getStartPoint(i))/(2*res[i]));
						//System.out.println("se l - "+i+"  "+select[i]);
						sel1[i] = 0.01;
						sel2[i] = 0.99;	
					}
				//}
				...
				{
					String qText;
					String relation = queryT.getRelationName(i);
					String relation1 = relation+select1;String relation2 = relation+select2;
					System.out.println(" Relation "+i+" - "+relation);
					String attribute = queryT.getAttribName(i);
					System.out.println(" Attribute "+i+" - "+attribute);
					qText = queryT.generateQuery(sel1);
					System.out.println(" query 1 "+qText);
					
					Statement stmt = database.createStatement();
					
					String attr_const1 = queryT.getHistogram(i).getConstant(sel1[i]);
					String attr_const2 = queryT.getHistogram(i).getConstant(sel2[i]);
					//qText = "create view "+relation1+" WITH SCHEMABINDING as select "+relation+".* from "+relation+" where "+relation+"."+attribute+" <= "+attr_const1;
					qText = "select "+relation+".* into "+relation1+" from "+relation+" where "+relation+"."+attribute+" <= "+attr_const1;
					System.out.println(" query 1 "+qText);
					//stmt.executeUpdate(qText);
					
					String stat = new String();
					stat = "STAT_"+relation1;
					qText = "CREATE STATISTICS "+stat+" ON "+relation1+" ( "+attribute+" )WITH FULLSCAN";				
					System.out.println(" stat 1 "+qText);
					//stmt.executeUpdate(qText);
					
					
					//qText = "create view "+relation2+" as select "+relation+".* from "+relation+" where "+relation+"."+attribute+" <= "+attr_const2;
					qText = "select "+relation+".* into "+relation2+" from "+relation+" where "+relation+"."+attribute+" <= "+attr_const2;
					System.out.println(" query 2 "+qText);
					//stmt.executeUpdate(qText);
					
					stat = "STAT_"+relation2;
					qText = "CREATE STATISTICS "+stat+" ON "+relation2+" ( "+attribute+" ) WITH FULLSCAN";				
					System.out.println(" stat 2 "+qText);
					//stmt.executeUpdate(qText);					
					
					replyRelationNames[i][0] = relation1;
					replyRelationNames[i][1] = relation2;
				}
				 
				//ServerMessageUtil.sendStatusMessage(sock,reader,writer,0,"RQEP_CREATE_TEMP_TABLES - End",MessageIds.RQEP_CREATE_TEMP_TABLES);
				ServerMessageUtil.sendTempTableNames(sock, reader, writer, replyRelationNames);
				System.out.println("Processing message - Done");
				break;
			case MessageIds.RQEP_DELETE_TEMP_TABLES:
				/* Added by Nila - RQEP */
				System.out.println("Delete - Processing message - Start");
				ServerMessageUtil.sendStatusMessage(sock,reader,writer,0,"RQEP_DELETE_TEMP_TABLES - Start");
				String[][] tempTableNames = (String[][]) clientPacket.get("TempTableNames");
				System.out.println(" Delete Tables***");
				Statement stmt = database.createStatement();
				if(tempTableNames != null)
				{
					for(int ii=0;ii<tempTableNames.length;ii++)
					{
						System.out.println("dimension "+(ii+1));
						for(int j=0;j<tempTableNames[ii].length;j++)
						{
							System.out.print("relation "+tempTableNames[ii][j]+"   ");
							String qText = "Drop table "+tempTableNames[ii][j];
				...
				ServerMessageUtil.sendStatusMessage(sock,reader,writer,0,"RQEP_DELETE_TEMP_TABLES - End",MessageIds.RQEP_DELETE_TEMP_TABLES);
				System.out.println("Delete -  Processing message - Done");
				break;
...
//		}
	...
	private void sendPlanStrings(QueryPacket qp, ArrayList points) throws PicassoException {
		...
		Query query = Query.getQuery(qp,database);
		System.out.println("sending plan strings for qname -  " + qp.getQueryName());
	...
	private void sendPlanCosts(QueryPacket qp, String planStr, ArrayList points) throws PicassoException {
		...
		Query query = Query.getQuery(qp,database);
		System.out.println("sending plan cost for qname -  " + qp.getQueryName());
		query.genConstants(qp.getResolution(), qp.getDistribution(), qp.getStartPoint(), qp.getEndPoint());
		int d = qp.getDimension();
		int r[] = qp.getResolution();
		int [] sel = new int[d];
