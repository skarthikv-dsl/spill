...
//			dp.transposeDiagram();
	...
	HashMap<String, Integer> relationNameNumbering;
	void readPicassoDiagram() throws PicassoException
	{
		...
		long[] hashCodes = readAllTrees(gdo.getMaxPlanNumber());
		
        if(PicassoConstants.isSAVE)
        {
                ADP.setPlans(plans);
                ADP.setMaxPlanNumber(plans.size());
                try
                {
                        String fName = PicassoConstants.SAVE_PATH + sqp.getQueryName() + ".apkt";
                ...
                // Save Join Order
                try{
                	relationNameNumbering = new HashMap<String, Integer>();
                	String qt = ADP.queryPacket.getQueryTemplate();
                	int Start = qt.indexOf("FROM");
                	if( Start == -1) {
                		Start = qt.indexOf("from");
                	}
                	int end = qt.indexOf("WHERE");
                	if( end  == -1) {
                		end = qt.indexOf("where");
                	}
                	end--;                	
                	String fromList = qt.substring(Start,end);
                	fromList = fromList.substring(5,fromList.length());
                	if(fromList.contains("from")){
                		fromList = fromList.substring(fromList.indexOf("from"),fromList.length());
                		fromList = fromList.substring(5,fromList.length());
                	}
                	if(fromList.contains("FROM")){
                		fromList = fromList.substring(fromList.indexOf("FROM"),fromList.length());
                	...
                	String[] array = fromList.split(",");
                	for(int i=0;i<array.length;i++) {
                		relationNameNumbering.put(array[i].trim().toLowerCase(), (i+1));
                	}
                	Iterator iter = trees.iterator();
                	Integer noPlans = (Integer) iter.next();
                	System.out.println("Number of Plans: "+noPlans);
                	
                	PrintWriter pw = null;
                	if(PicassoConstants.isSAVE)		                		pw = new PrintWriter(new File(PicassoConstants.SAVE_PATH + ADP.queryPacket.getQueryName() + "\\plans"));
                	
                	while(iter.hasNext()) {
                		Integer planNumber = (Integer) iter.next();
                		TreeNode root = (TreeNode) iter.next();
                		String joinOrder = "";
                		joinOrder = joinOrder + this.getJoinOrder(root);
                		System.out.println("Plan Number: "+planNumber+" JoinOrder:"+joinOrder );
//                		System.out.println(""+joinOrder );
                		if(PicassoConstants.isSAVE)                			pw.println(joinOrder);
                	}
                	
                	if(PicassoConstants.isSAVE)    		            		pw.close();
		...
		ServerMessageUtil.sendPlanDiagramWithHashcodes(sock, reader, writer, queryPacket, gdo, trees, hashCodes);
	}
	...
	public String getJoinOrder(TreeNode root) {
    	String join="";
    	...
    	if(root.getChildren().size() == 0) {
    		...
    		Integer RelNumber = relationNameNumbering.get(nodeName.toLowerCase());
    		if(RelNumber != null) nodeName = RelNumber +"";
    		else				  nodeName = "";		
    		
    		//if(root.getNodeName().contains("pkey"))
    			//System.out.println((root.getParent()).getNodeName()+" "+root.getNodeName());
    		if(root.getNodeName().equals("XSCAN") || root.getNodeName().equals("GENROW")
    				|| root.getNodeName().equals("XUNNEST") || root.getNodeName().contains("pkey") || root.getNodeName().contains("_idx") ) {
    			return "*";
    		}
    		else
    		{
    	...
    	if(root.getChildren().size() == 1) {
    		Vector temp = root.getChildren();
    		return getJoinOrder((TreeNode)temp.get(0));
    	}
    	
    	if(root.getChildren().size() == 2) {
//    		if(root.getNodeName().compareTo("Merge Join")==0)
//    			System.out.println(root.getNodeName());
    		Vector children = root.getChildren();
    		String[] joinOrders = new String[root.getChildren().size()];
//    		join = "(";
    		for(int i=0 ; i < root.getChildren().size(); i++) {
    			joinOrders[i]=getJoinOrder((TreeNode)children.get(i));
    		}
    		if(joinOrders[0].equals("*")) {
    			if(joinOrders[1].equals("*")) {
    				return "*";
    			}
    			else
    				return joinOrders[1];
    		}
    		else if(joinOrders[1].equals("*")) {
    			return joinOrders[0];
    			}
    		else {
 			    			
//    			if(joinOrders[0].compareTo("("+joinOrders[1]+")") == 0)
//    				join = joinOrders[1];
//    			else if(joinOrders[1].compareTo("("+joinOrders[0]+")") == 0)
//    				join = joinOrders[0];
//    			else if(joinOrders[0].compareTo(joinOrders[1]) == 0)
//    				join = joinOrders[0];
//    			else
//    				join="("+joinOrders[0]+""+joinOrders[1]+")";
    			
    			joinOrders[0] = clean(joinOrders[0]);
    			joinOrders[1] = clean(joinOrders[1]);

    			if(joinOrders[0].compareTo(joinOrders[1]) == 0
    					|| joinOrders[0].contains(joinOrders[1]))
    				join = joinOrders[0];
    			else if( joinOrders[1].contains(joinOrders[0]) )
    				join = joinOrders[1];
    			else{
//    				if(root.getNodeName().contains("nest") || root.getNodeName().contains("Nest"))
//    					join="("+joinOrders[0]+"N"+joinOrders[1]+")";
//    				else if(root.getNodeName().contains("merge") || root.getNodeName().contains("Merge"))
//    					join="("+joinOrders[0]+"M"+joinOrders[1]+")";
//    				if(root.getNodeName().contains("hash") || root.getNodeName().contains("Hash"))
//    					join="("+joinOrders[0]+"H"+joinOrders[1]+")";
    	...
    	if(root.getChildren().size() >2) {
    		System.out.println("There are more than 2 inputs!");
//    		if(root.getNodeName().compareTo("Merge Join")==0)
//    			System.out.println(root.getNodeName());
    		Vector children = root.getChildren();
    		String[] joinOrders = new String[root.getChildren().size()];
...
//		plans = new Vector();
		...
		try
		{
			...
			ADP = ADiagramPacket.createDiagramPacket(dimension,resolution,relNames,attribNames,attribTypes,data,
					max,min,picsel,plansel,predsel,datasel,constants,queryPacket,false);
...
//			}
	...
	// Returns the hashCode (in proper order) - RQEP Nila
	public long[] readAllTrees(int maxPlan) throws PicassoException
	{
		long[] hashCode = new long[maxPlan];
		int qtid = database.getQTID(queryPacket.getQueryName());
		...
		for (int i=0; i < maxPlan; i++) {
			...
			plan.computeMSSQLHash(PicassoConstants.SUBOPERATORLEVEL);
			hashCode[i] = plan.getHash();
			if(plan!=null)
			{
