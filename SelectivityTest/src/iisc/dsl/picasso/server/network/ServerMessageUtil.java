...
//		we'll send compressed trees instead
	...
	// Added by Nila - RQEP
	public static final void sendPlanDiagramWithHashcodes(Socket sock, ObjectInputStream in, 
			ObjectOutputStream out, QueryPacket qp, DiagramPacket dp, Vector trees, long[] hashCodes) {
...
//		we'll send compressed trees instead
	...
	public static final void sendTempTableNames(Socket sock, ObjectInputStream in, 
			ObjectOutputStream out, String[][] tempRealtionNames) {
		if ( sock == null )
			return;
		
		ServerPacket packet = new ServerPacket();
		packet.messageId = MessageIds.RQEP_CREATE_TEMP_TABLES;
		packet.status = MessageIds.END_MESSAGE;
		packet.hashmap = new HashMap();
		packet.hashmap.put("TempTableNames", tempRealtionNames);
