...
import iisc.dsl.picasso.common.ds.*;
import iisc.dsl.picasso.server.plan.Plan;


public class ADiagramPacket implements Serializable {
...
//        ArrayList                 PlanTrees;
        ...
        //Routines to manipulate the information in the ADiagramPacket follow.
        
    public ADiagramPacket()
    {
            
    }
    
    public ADiagramPacket(ADiagramPacket p)
    {
    ...
    public DataValues[] copydata(DataValues[] source)
    {
     ...
     /* Function: createADiagramPacket
         ...
         */
        public static ADiagramPacket createADiagramPacket(int dimension, int resolution[],
                                                String[] relationNames, String attrNames[], DataValues[] dataPoints,
DataValues maxValues, DataValues minValues,
                                                float picsel[], float plansel[], float predsel[], float datasel[],String
constants[], QueryPacket qp,boolean approxFlag) 
        {
                
                // Create the packet...
                ADiagramPacket packet = new ADiagramPacket();
        ...
        public static ADiagramPacket createDiagramPacket(int dimension, int resolution[],
                        String[] relationNames, String attrNames[], String attrTypes[], DataValues[]
dataPoints, DataValues maxValues, DataValues minValues,
                        float picsel[], float plansel[], float predsel[], float datasel[],String
constants[], QueryPacket qp,boolean approxFlag) 
        {
        ...
        public void transposeDiagram()
        {
                ...
                relationNames[0] = tmp;
                DataValues [] tdata = new DataValues [resolution[PicassoConstants.a[0]] *
resolution[PicassoConstants.a[1]]];
                tmp =attributes[1];
                ...
                if(dimension > 2)
                {
                        ...
                        for(j = 0; j < resolution[PicassoConstants.a[1]]; j++)
                                for(i = j; i <
resolution[PicassoConstants.a[1]]*resolution[PicassoConstants.a[0]];
i+=resolution[PicassoConstants.a[1]])
                ...
                else // Exponential
                {
                        ...
                        for (int i=0; i < dataPoints.length; i++)
                        {
                                ...
                                if(dim == 1)
                                {
                                        ...
                                        {
                                                curval=getQueryPacket().getEndPoint(0)*100-selvals[getResolution(0)-1] +
(selvals[getResolution(0)-1]-selvals[getResolution(0)-2])/2;
                                        }
                                        fullval = curval;
                                }
                                else if(dim == 2)
                                {
                                        for(we = 0; we < dim; we++)
                                        {
                                                ...
                                                {
                                                        curval=(selvals[ressum[PicassoConstants.a[we]]+
idx[we]+1]-selvals[ressum[PicassoConstants.a[we]]+ idx[we]-1])/2;
                                                }
                                                else if(idx[we]==0)
                                                {
                                                ...
                                                {
                                                        curval=getQueryPacket().getEndPoint(PicassoConstants.a[we])*100-selvals[ressum[PicassoConstants.a[we]]+getResolution(PicassoConstants.a[we])-1]
+
(selvals[ressum[PicassoConstants.a[we]]+getResolution(PicassoConstants.a[we])-1]-selvals[ressum[PicassoConstants.a[we]]+
getResolution(PicassoConstants.a[we])-2])/2;
                                                }
                                                if(!scaleupflag)
                                                        fullval*=curval;
                                                else
                                ...
                                else
                                {
                                        ...
                                        for(we=0;we<dim;we++)
                                        {
                                                ...
                                                {
                                                        curval=endpt[we]*100-selvals[res[we]-1] +
(selvals[res[we]-1]-selvals[res[we]-2])/2;
                                                }
